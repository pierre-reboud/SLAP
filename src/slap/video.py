...
class AbstractVideo(ABC):
    def __init__(self, configs: Configs):
        ...
        if self.configs.video_name == "video":
            self.capture : cv2.VideoCapture = cv2.VideoCapture(self.configs.video_path)
        self.frame_count : int = int(self.capture.get(cv2.CAP_PROP_FRAME_COUNT)) if self.configs.video_name == "video" else 3000
        self.frame_W : int = int(self.capture.get(cv2.CAP_PROP_FRAME_WIDTH)) if self.configs.video_name == "video" else self.configs.frame_w
        self.frame_H : int = int(self.capture.get(cv2.CAP_PROP_FRAME_HEIGHT)) if self.configs.video_name == "video" else self.configs.frame_h
        self.stream = self.get_stream(self.configs.video_path)
...
class Video(AbstractVideo):
    ...
    def get_stream(self, video_path: str) -> Generator[Tuple[np.ndarray, int, int], str, None]:
        ...
        Yields:
            Generator[Tuple[np.ndarray, int, int], str, None]: _description_
        """
        if self.configs.video_name == "video.mp4":              
            ...
            while (frame_counter < self.frame_count and frame_retrieved):
                ...
                frame_counter += 1
            capture.release()
        elif self.configs.video_name == "rgb":
            frame_names = sorted(os.listdir(video_path))
            frame_counter : int = 0
            frame_retrieved : bool = True
            frame : np.ndarray
            while frame_names:
                frame = cv2.imread(self.configs.video_path + "/" + frame_names.pop(0))
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) if self.configs.grey else frame
                frame = cv2.undistort(frame, self.intrinsics, self.distortion_coefs)
                self.frames_buffer[frame_counter%3] = frame
                yield frame, frame_counter%3, (frame_counter-1)%3
